...
// Existing Token-2022 mint on mainnet
const EXISTING_TOKEN_MINT = new PublicKey(
  "BuRbwqMMjUkp3VKyprr3TqFD6ng71Pm8bfuei6ohfast"
);

// Existing Metaplex Core Collection on mainnet
const EXISTING_COLLECTION = new PublicKey(
  "43wJ2YA1qdKfYv1GUUnCvT8b2ksZtNEx1Ns6suJa5c1C"
);
...
async function main() {
  console.log("===========================================");
  console.log("  Billion Grid - MAINNET Initialization");
  console.log("===========================================\n");
  ...
  // ============================================
  // Step 1: Use Existing Token Mint
  // ============================================
  console.log("Step 1: Using existing Token-2022 Mint...");
  const tokenMint = EXISTING_TOKEN_MINT;
  ...
  await trackCost(provider, "Create BlockMap Account", async () => {
  ...
  // PRODUCTION URI
  const uriBase = "https://million-dollar-api.seva-e96.workers.dev/parcel";
  ...
  await trackCost(provider, "Initialize Grid Config", async () => {
  ...
  // ============================================
  // Step 4: Transfer Collection Ownership to GridConfig PDA
  // ============================================
  console.log(
    "\nStep 4: Transferring Collection Ownership to GridConfig PDA..."
  );
  console.log("   Collection:", EXISTING_COLLECTION.toBase58());
  console.log("   New Update Authority:", gridConfigPda.toBase58());

  const umi = createUmi(provider.connection.rpcEndpoint);
  const umiKeypair = fromWeb3JsKeypair(authority.payer);
  umi.use(keypairIdentity(umiKeypair));

  const collectionUmi = publicKey(EXISTING_COLLECTION.toBase58());
  const gridConfigPdaUmi = publicKey(gridConfigPda.toBase58());

  await trackCost(provider, "Transfer Collection Ownership", async () => {
    await updateCollectionV1(umi, {
      collection: collectionUmi,
      newUpdateAuthority: gridConfigPdaUmi,
    }).sendAndConfirm(umi);
    console.log("   Collection ownership transferred!");
  });
  ...
  await trackCost(provider, "Update Config with Collection", async () => {
    const sig = await program.methods
      .updateConfig(
        null,
        null,
  ...
  console.log(
    `https://explorer.solana.com/address/${program.programId.toBase58()}`
  );
}

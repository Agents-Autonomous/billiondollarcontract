...
// D1 database binding name (from wrangler.toml - PRODUCTION)
const D1_DATABASE = "million-dollar-db";
...
function queryUnclaimedParcels(): DbParcel[] {
  console.log("Querying PRODUCTION D1 for unclaimed parcels...");
  ...
  try {
    ...
    const result = execFileSync(
      "npx",
      ["wrangler", "d1", "execute", "DB", "--remote", "--env", "production", "--command", query, "--json"],
      {
        cwd: WORKER_DIR,
        encoding: "utf-8",
...
function updateParcelOnChainId(parcelDbId: string, onChainParcelId: number): void {
  ...
  try {
    ...
    execFileSync(
      "npx",
      ["wrangler", "d1", "execute", "DB", "--remote", "--env", "production", "--command", query],
      {
        cwd: WORKER_DIR,
        encoding: "utf-8"
      }
...
async function main() {
  ...
  if (!gridConfig.seedingEnabled) {
    ...
    console.log("  program.methods.updateConfig(null, null, null, true, null, null, null)");
    process.exit(1);
  }

  // Process each parcel
  console.log("===========================================");
  console.log("  Processing Parcels (MAINNET)");
  console.log("===========================================\n");
  ...
  for (let i = 0; i < parcels.length; i++) {
    ...
    const shouldMint = await confirm(`    Mint this parcel on MAINNET?`);
    if (!shouldMint) {
      console.log(`    Skipped.\n`);
